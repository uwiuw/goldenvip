...
class Main extends CI_Controller {
    ...
    function list_distributor() {
        ...
        $data['limit'] = $limit;
        $data['add_new'] = "<a href='javascript:void(load(\"_admin/distributor/add_new\",\"#site-content\"))' class='button'>add new distributor</a>";
    ...
    function add_new_distributor() {
        ...
        $this->load->view('panel/page/distributor/add_new_distributor', $data);
    }

    function saving_new_member() {
        is_admin();
        $u = $this->Mix->read_row_by_one('username', $this->input->post('username'), 'tx_rwmembermlm_member');
        
        if (!empty($u)):
            echo "Sorry username is already exists, please try again.";
        else:
            $e = $this->Mix->read_row_by_one('email', $this->input->post('email'), 'tx_rwmembermlm_member');
            if (!empty($e)):
                echo "Sorry email already exists, please try again.";
            else:
                $data['pid'] = 67;
                ...
                $fe_user['username'] = $this->input->post('username');
                $fe_user['password'] = md5($this->input->post('password'));
                $fe_user['pid'] = 67;
                $fe_user['usergroup'] = 3;
                $fe_user['email'] = $this->input->post('email');
                $fe_user['crdate'] = mktime(date('H'), date('i'), date('s'), date('m'), date('d'), date('y'));
                $fe_user['deleted'] = 0;
                $this->Mix->add_with_array($fe_user, 'fe_users');
                echo "New Distributor has been saved.";
            endif;
        endif;
    }
